...
def main():
    ...
    train_order = np.repeat(train_order, 9)
    test_order = np.repeat(test_order, 9)
    print("train_order.shape: {}".format(train_order.shape))
    print("test_order.shape: {}".format(test_order.shape))

    train_image_order = np.array(range(9) * train_cnt)
    test_image_order = np.array(range(9) * test_cnt)
    print("train_image_order.shape: {}".format(train_image_order.shape))
    print("test_image_order.shape: {}".format(test_image_order.shape))
    ...
    train_image_order_cluster_root = []
    test_image_order_cluster_root = []
    ...
    zip_train_image_order_cluster = zip(train_image_order, train_cluster_label)
    zip_test_image_order_cluster = zip(test_image_order, test_cluster_label)
    for i in range(n_clusters):
        ...
        train_image_order_cluster_root.append(np.array([order for order, m in zip_train_image_order_cluster if m == i]))
        test_image_order_cluster_root.append(np.array([order for order, m in zip_test_image_order_cluster if m == i]))
    ...
    print(train_image_order_cluster_root[0].shape)
    print(train_image_order_cluster_root[1].shape)
    print(test_image_order_cluster_root[0].shape)
    print(test_image_order_cluster_root[1].shape)

    train_coefficient = np.empty([train_cnt, 92 * 9])
    test_coefficient = np.empty([test_cnt, 92 * 9])
    for n in range(32):
        ...
        train_image_order = train_image_order_cluster_root[tree[0]]
        test_image_order = test_image_order_cluster_root[tree[0]]
        means = np.mean(train_block, axis=0).reshape(1, -1)
        print("means.shape: {}".format(means.shape))
        std = np.std(train_block, axis=0).reshape(1, -1)
        print("std.shape: {}".format(std.shape))
        ...
        train_order_cluster = []
        test_order_cluster = []
        train_image_order_cluster = []
        test_image_order_cluster = []
        zip_train_cluster = zip(train_block, train_cluster_label)
        zip_test_cluster = zip(test_block, test_cluster_label)
        # zip_train_label_cluster = zip(train_label, train_cluster_label)
        zip_train_order_cluster = zip(train_order, train_cluster_label)
        ...
        for i in range(n_clusters):
            ...
            train_image_order_cluster.append(np.array([order for order, m in zip_train_image_order_cluster if m == i]))
            test_image_order_cluster.append(np.array([order for order, m in zip_test_image_order_cluster if m == i]))
        ...
        print(train_image_order_cluster[0].shape)
        print(train_image_order_cluster[1].shape)
        print(test_image_order_cluster[0].shape)
        print(test_image_order_cluster[1].shape)
        ...
        train_image_order = train_image_order_cluster_root[tree[1]]
        test_image_order = test_image_order_cluster_root[tree[1]]
        means = np.mean(train_block, axis=0).reshape(1, -1)
        print("means.shape: {}".format(means.shape))
        std = np.std(train_block, axis=0).reshape(1, -1)
        print("std.shape: {}".format(std.shape))
        ...
        train_image_order = train_image_order_cluster_root[tree[2]]
        test_image_order = test_image_order_cluster_root[tree[2]]
        means = np.mean(train_block, axis=0).reshape(1, -1)
        print("means.shape: {}".format(means.shape))
        std = np.std(train_block, axis=0).reshape(1, -1)
        print("std.shape: {}".format(std.shape))
        ...
        train_image_order = train_image_order_cluster_root[tree[3]]
        test_image_order = test_image_order_cluster_root[tree[3]]
        means = np.mean(train_block, axis=0).reshape(1, -1)
        print("means.shape: {}".format(means.shape))
        std = np.std(train_block, axis=0).reshape(1, -1)
        print("std.shape: {}".format(std.shape))
        ...
        train_image_order = train_image_order_cluster_root[tree[4]]
        test_image_order = test_image_order_cluster_root[tree[4]]
        means = np.mean(train_block, axis=0).reshape(1, -1)
        print("means.shape: {}".format(means.shape))
        std = np.std(train_block, axis=0).reshape(1, -1)
        print("std.shape: {}".format(std.shape))
        ...
        train_image_order_0 = train_image_order_cluster[0]
        train_image_order_1 = train_image_order_cluster[1]
        ...
        test_image_order_0 = test_image_order_cluster[0]
        test_image_order_1 = test_image_order_cluster[1]
        ...
        for p in range(train_order_0.shape[0]):
            train_coefficient[train_order_0[p], train_image_order_0[p] * 92:(train_image_order_0[p] + 1) * 92] = train_coefficient_0[p, :]

        for p in range(train_order_1.shape[0]):
            train_coefficient[train_order_1[p], train_image_order_1[p] * 92:(train_image_order_1[p] + 1) * 92] = train_coefficient_1[p, :]

        for p in range(test_order_0.shape[0]):
            test_coefficient[test_order_0[p], test_image_order_0[p] * 92:(test_image_order_0[p] + 1) * 92] = test_coefficient_0[p, :]

        for p in range(test_order_1.shape[0]):
            test_coefficient[test_order_1[p], test_image_order_1[p] * 92:(test_image_order_1[p] + 1) * 92] = test_coefficient_1[p, :]
