...
def window_process(train, test, size):
...
def convolution(train, test, components, size):
    ...
    train_window, test_window = window_process(train, test, size)
    # pca training
...
def main():
    ...
    train, test = convolution(train, test, 3, 2)
    print("train.shape: {}".format(train.shape))
    print("test.shape: {}".format(test.shape))
    ...
    train, test = convolution(train, test, 4, 2)
    print("train.shape: {}".format(train.shape))
    print("test.shape: {}".format(test.shape))
    train_data = np.concatenate((train_data, Unsign(train)), axis=1)
    test_data = np.concatenate((test_data, Unsign(test)), axis=1)
    # train_data = train.reshape(train.shape[0], -1)
    # test_data = test.reshape(test.shape[0], -1)
    print("train_data.shape: {}".format(train_data.shape))
    print("test_data.shape: {}".format(test_data.shape))

    train, test = convolution_overlapping(train, test, 7, 4)
    print("train.shape: {}".format(train.shape))
    print("test.shape: {}".format(test.shape))
    train_data = np.concatenate((train_data, Unsign(train)), axis=1)
    test_data = np.concatenate((test_data, Unsign(test)), axis=1)
    ...
    train, test = convolution(train, test, 6, 5)
    print("train.shape: {}".format(train.shape))
    print("test.shape: {}".format(test.shape))
    train_data = np.concatenate((train_data, Unsign(train)), axis=1)
    test_data = np.concatenate((test_data, Unsign(test)), axis=1)
