...
def main():
    ...
    train_probability = np.load('./train_classifier' + '.npy')
    test_probability = np.load('./test_classifier' + '.npy')
    print("train_probability.shape: {}".format(train_probability.shape))
    print("test_probability.shape: {}".format(test_probability.shape))
    train_size = train_probability.shape[0]
    test_size = test_probability.shape[0]
    print("train_size.shape: {}".format(train_size))
    print("test_size.shape: {}".format(test_size))
    train_new_label = np.empty(train_size)
    test_new_label = np.empty(test_size)
    for i in range(train_size):
        prob_n = np.argsort(-train_probability[i])
        train_probability[i] = np.array(sorted(train_probability[i], reverse=True))
        if prob_n[0] == train_label[i]:
            train_new_label[i] = 1
        elif prob_n[1] == train_label[i]:
            train_new_label[i] = 2
        elif prob_n[2] == train_label[i]:
            train_new_label[i] = 3
        else:
            train_new_label[i] = 4
    for j in range(test_size):
        prob_n = np.argsort(-test_probability[j])
        test_probability[j] = np.array(sorted(test_probability[j], reverse=True))
        if prob_n[0] == test_label[j]:
            test_new_label[j] = 1
        elif prob_n[1] == test_label[j]:
            test_new_label[j] = 2
        elif prob_n[2] == test_label[j]:
            test_new_label[j] = 3
        else:
            test_new_label[j] = 4

    # pca = PCA(n_components=4)
    # pca.fit(train_probability)
    # train_probability = pca.transform(train_probability)
    # test_probability = pca.transform(test_probability)

    # classifier = svm.SVC(class_weight='balanced')
    # classifier.fit(train_probability, train_new_label)
    # test_predict = classifier.predict(test_probability)
    # train_predict = classifier.predict(train_probability)
    ...
    print(list(test_predict).count(1))
    print(list(test_predict).count(2))
    print(list(test_predict).count(3))
    print(list(test_predict).count(4))
    train_predict = np.load("./train_predict.npy")
    train_size = train_predict.shape[0]
    for i in range(train_size):
        if train_predict[i] == train_new_label[i]:
            continue
        else:
    ...
    for i in range(10000):
        print(i)
        if test_predict[i] == 1:
            accuracy_test += list(test_probability[i]).index(max(test_probability[i])) == test_label[i]
        elif test_predict[i] == 3 or test_predict[i] == 2 or test_predict[i] == 4:
            count += 1
            max_index_1 = np.argsort(-test_probability[i])[0]
            max_index_2 = np.argsort(-test_probability[i])[1]
            ...
            if element not in confusion_labels:
                ...
                size_11 = data_11.shape[0]
                size_12 = data_12.shape[0]
                for m in range(data_11.shape[1]):
                    hist_11, edges_11 = np.histogram(data_11[:, m] / size_11, bins=10)
                    hist_12, edges_12 = np.histogram(data_12[:, m] / size_12, bins=10)
                ...
                size_21 = data_21.shape[0]
                size_22 = data_22.shape[0]
                for m in range(data_21.shape[1]):
                    hist_21, edges_21 = np.histogram(data_21[:, m] / size_21, bins=10)
                    hist_22, edges_22 = np.histogram(data_22[:, m] / size_22, bins=10)
                    score_2.append(entropy(hist_22, hist_21))
    ...
    # zip_data = zip(train_new, train_label_new)
    # dataset = []
    # for i in range(10):
    ...
    #             train_dataset = np.array(dataset[i])
    #             test_dataset = test[:]
    #             for k in range(5):
    #                 stage = k + 1
    #                 print("The {} stage: ".format(stages[k]))
    ...
    #             np.save("./new/test" + str(i) + ".npy", temper)
    #         else:
    #             for k in range(5):
    #                 stage = k + 1
    #                 print("The {} stage: ".format(stages[k]))
    ...
    #                 else:
    #                     temp = np.concatenate((temp, Unsign(test_n)), axis=1)
    #         np.save("./new/train" + str(i) + str(j) + ".npy", temp)
    ...
    #             for m in range(data_11.shape[1]):
    #                 score_1.append(max_data_11[m] - max_data_12[m])
    #             score_1 = np.array(score_1)
    #             count_1 = sum(score_1 > 0)
    #             index_1 = np.argsort(-score_1)[:count_1]
    #             data_1 = data_1[:, index_1]
    ...
    #             for m in range(data_21.shape[1]):
    #                 score_2.append(max_data_22[m] - max_data_21[m])
    #             score_2 = np.array(score_2)
    #             count_2 = sum(score_2 > 0)
    #             index_2 = np.argsort(-score_2)[:count_2]
    #             data_2 = data_2[:, index_2]
