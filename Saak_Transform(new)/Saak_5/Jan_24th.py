...
def convolution_2(train, test, stage):
    ...
    train_filter = patch_filter(train_window)
    # pca training
...
def convolution_1(sample, stage):
    ...
    w, h, d = sample_shape[1], sample_shape[2], sample_shape[3]
    # use sample to do the DC, AC substraction
    sample_window = window_process_1(sample)
...
def cluster(sample, sample_label, stage):
    ...
    for i in range(10):
        data_1 = []
        for j in range(10):
            data_1.append(convolution_2(np.array(dataset[i]), np.array(dataset[j]), stage))
...
def main():
    ...
    for k in range(5):
        ...
        train, test = convolution_2(train, test, stage)
