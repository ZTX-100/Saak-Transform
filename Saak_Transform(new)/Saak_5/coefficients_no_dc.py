...
def Unsign(train_data):
    filternum = (train_data.shape[3]) / 2
    ta1 = train_data[:, :, :, :filternum] - train_data[:, :, :, filternum:]
...
def convolution_2(train, test, stage):
    ...
    train_data = np.zeros((train_cnt, w, h, d * 2))
    test_data = np.zeros((test_cnt, w, h, d * 2))
    ...
    for i in range(10):
        ...
        for j in range(10):
            ...
            if j == 0:
                ...
                np.save("./coefficients_no_dc/test" + str(i) + ".npy", temper)
            else:
                for k in range(5):
                    stage = k + 1
                    print("The {} stage: ".format(stages[k]))
                    ...
                    if k == 0:
                        ...
                        temp = np.concatenate((temp, Unsign(test_n)), axis=1)
            np.save("./coefficients_no_dc/train" + str(i) + str(j) + ".npy", temp)
