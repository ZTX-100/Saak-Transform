...
def main():
    ...
    #             for t in range(test_index.shape[0]):
    #                 train_dataset = np.array(dataset[i])
    #                 test_dataset = test[test_index, :]
    #                 test_1 = np.expand_dims(test_dataset[t], axis=0)
    #                 for k in range(5):
    #                     stage = k + 1
    #                     print("The {} stage: ".format(stages[k]))
    #                     train_dataset, test_1 = convolution_2_pca(train_dataset, test_1, stage, pca[k])
    #                     if k == 0:
    #                         temper = Unsign(test_1)
    #                     else:
    #                         temper = np.concatenate((temper, Unsign(test_1)), axis=1)
    #                 if t == 0:
    #                     test_temper = temper
    #                 else:
    #                     test_temper = np.concatenate((test_temper, temper), axis=0)
    #             np.save("./140/test" + str(i) + ".npy", test_temper)
    ...
    #                 else:
    #                     temp = np.concatenate((temp, Unsign(test_n)), axis=1)
    #         np.save("./140/train" + str(i) + str(j) + ".npy", temp)
