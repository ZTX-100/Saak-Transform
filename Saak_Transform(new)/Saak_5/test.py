...
def main():
    ...
    #             train_n, test_n = convolution_2(train_n, test_n, stage)
    #             train_dataset, test_dataset = convolution_2(train_dataset, test_dataset, stage)
    # if k == 0:
    # temp = Unsign(test_n)
    ...
    #     np.save("./5_30/train" + str(i) + str(j) + ".npy", temp)
    # np.save("./5_30/test" + str(i) + ".npy", temper)
    #     if j == 0:
    #         data_n = temp
    #     else:
    #         data_n = np.concatenate((data_n, temp), axis=0)
    # data_all.append(data_n)

    # for i in range(10):
    #     select = SelectPercentile(f_classif, percentile=75)
    #     data_one = select.fit_transform(data_all[i], data_labels)
    #     pca = PCA(n_components=64)
    #     data_one = pca.fit_transform(data_one)
    #     for j in range(10):
    #         np.save("./100/train" + str(i) + str(j) + ".npy", data_one[num[j]:num[j + 1]])

    # test_pca = np.load('./test_pca' + '.npy')
    # print("test_pca.shape: {}".format(test_pca.shape))
    ...
    for i in range(10000):
        print(i)
        test_list = list(test_probability[i])
        max_1 = max(test_list)
        max_index_1 = test_list.index(max_1)
        test_list.remove(max_1)
        max_2 = max(test_list)
        test_list.insert(max_index_1, max_1)
        max_index_2 = test_list.index(max_2)
        if max_1 / max_2 > 2:
            ...
            if element not in confusion_labels:
                ...
                select_1 = SelectPercentile(f_classif, percentile=75)
                data_1 = select_1.fit_transform(np.vstack((data_11, data_12)), np.concatenate((max_index_1 * np.ones(data_11.shape[0]), max_index_2 * np.ones(data_12.shape[0]))))
                ...
                select_2 = SelectPercentile(f_classif, percentile=75)
                data_2 = select_2.fit_transform(np.vstack((data_21, data_22)), np.concatenate((max_index_1 * np.ones(data_21.shape[0]), max_index_2 * np.ones(data_22.shape[0]))))
                ...
                select.append([select_1, select_2])
            else:
                ...
                select_1 = select[confusion_labels.index(element)][0]
                select_2 = select[confusion_labels.index(element)][1]

            test_pca_1 = select_1.transform(test_1[i].reshape(1, -1))
            test_pca_2 = select_2.transform(test_2[i].reshape(1, -1))
