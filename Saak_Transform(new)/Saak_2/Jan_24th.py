...
def window_process(sample, train, test, sample_label, train_label, test_label):
    ...
    print("sample_labels.shape: {}".format(sample_label.shape))
...
def convolution(train, test, train_label, test_label, k, stage, per=None, class_id=-1):
    # generate sample data and label, change 60000 -> other number (number of images to learn PCA)
    #   sample, sample_label = generate_sample(train, train_label, 500000, 10)
    if class_id == 0:
        sample = train[train_label == 1]
        sample_label = train_label[train_label == 1]
    elif class_id == 1:
        sample = train[train_label == 6]
        sample_label = train_label[train_label == 6]
...
def main():
    ...
    train_all = train_all[np.any([train_label_all == 1, train_label_all == 6], axis=0)]
    test_all = test_all[np.any([test_label == 1, test_label == 6], axis=0)]
    train_label_all = train_label_all[np.any([train_label_all == 1, train_label_all == 6], axis=0)]
    test_label = test_label[np.any([test_label == 1, test_label == 6], axis=0)]
    ...
    for class_id in range(2):
        ...
        print class_id, train_cnt, test_cnt
        ...
        for k in [-1, -1, -1, -1, -1]:
            ...
            train, test = convolution(train, test, train_label, test_label, k, stage, Percentage[stage - 1], class_id)
            print_shape(train, test)
            # save features of each stage (augmented features, when do classfication, you need to converge)
            np.save('./feature/train_before_f_test_class' + str(class_id) + '_' + str(stage) + '_v' + str(version) + '.npy', train)
            np.save('./feature/test_before_f_test_class' + str(class_id) + '_' + str(stage) + '_v' + str(version) + '.npy', test)
            stage += 1
