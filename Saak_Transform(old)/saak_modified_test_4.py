...
def meanshift_cluster(data_in):
    ...
    global ratio
    data = np.reshape(data_in, (data_in.shape[0], -1))
    # count_choice = np.random.choice(np.arange(data.shape[0]), size=int(data.shape[0] * 0.002), replace=False)
    count_choice = range(int(data.shape[0] * ratio))
    bandwidth = estimate_bandwidth(X=data[count_choice, :], quantile=0.6)
    kmeans = MeanShift(bandwidth=bandwidth, bin_seeding=True).fit(data[count_choice, :])
...
def saak_coefficients(data_flatten=None, input_channels=0, components_pca=0):
    ...
    print("PCA_and_augment: data_0: {}".format(data.shape))
