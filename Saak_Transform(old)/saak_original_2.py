...
def PCA_and_augment(data_in):
    ...
    N = comps.shape[1]
    dc = np.ones([1, N]) * (N**0.5)
    # comps_aug = [vec * (-1) for vec in comps[:-1]]
    # comps_complete = np.vstack((comps, comps_aug))
    comps_complete = dc
    for vec in comps:
        comps_complete = np.vstack((comps_complete, vec))
        comps_complete = np.vstack((comps_complete, vec * (-1)))
    # comps_complete = comps_complete[1:, :]
    comps = np.vstack((dc, comps))
...
def one_stage_saak_trans(datasets=None, depth=0, components_pca=0):
    ...
    comps, comps_complete = PCA_and_augment(data_flatten)
    comps = comps[:components_pca + 1, :]
    comps_complete = comps_complete[:1 + 2 * components_pca, :]
    print("one_stage_saak_trans: comps: {}".format(comps.shape))
    print("one_stage_saak_trans: comps_complete: {}".format(comps_complete.shape))

    # get filter, (7,1,2,2)
    filters = ret_filt_patches(comps, input_channels)
    filters_complete = ret_filt_patches(comps_complete, input_channels)
    print("one_stage_saak_trans: filters: {}".format(filters.shape))
    print("one_stage_saak_trans: filters_complete: {}".format(filters_complete.shape))

    # output (60000,7,14,14)
    output, relu_output = conv_and_relu(filters, datasets, stride=2)
    output_complete, relu_output_complete = conv_and_relu(filters_complete, datasets, stride=2)
    data = relu_output_complete.data.numpy()
