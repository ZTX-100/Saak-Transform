...
def main():
    ...
    zip_data = zip(train_coefficients_f_test, train_labels)
    train_mean = []
    for i in range(10):
        train_mean.append(list(np.mean([dat for dat, k in zip_data if k == i], axis=0)))

    acc = 0
    for j in range(test_size):
        distance = euclidean_distances(train_mean, np.expand_dims(test_coefficients_f_test[j, :], axis=0))
        index = list(distance).index(min(distance))
        if index == test_labels[j]:
            acc += 1
    end_time = time()
    ...
    print ("The accuracy of testing set is {:.4f}".format(acc))
