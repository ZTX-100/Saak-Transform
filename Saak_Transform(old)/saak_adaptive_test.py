...
def conv_and_relu(filters, datasets):
...
def one_stage_saak_trans(datasets=None, depth=0, components_pca=0):
    ...
    # var += 1
    data_flatten = fit_pca_shape(datasets, depth)
    data = np.reshape(data_flatten, (data_flatten.shape[0], -1))
    ...
    filters_augment = np.expand_dims(comps_complete, axis=1)

    print("one_stage_saak_trans: comps_complete: {}".format(comps_complete.shape))

    # output (60000,7,14,14)
    output_augment, relu_output_augment = conv_and_relu(filters_augment, data)
    relu_output_augment = relu_output_augment.data.numpy()
    relu_output_augment = np.squeeze(relu_output_augment)
