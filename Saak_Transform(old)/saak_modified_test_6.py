...
from sklearn.cluster import estimate_bandwidth, MeanShift, Birch
...
def meanshift_cluster_test(data_in):
    global var_test
    global global_K_test
    n_samples = data_in.shape[0]
    var_test += 1
    if var_test == 1:
        data = np.reshape(data_in, (n_samples, -1))
        data = list(data)
        for i in range(n_samples):
            data[i] = rearray(data[i])
        data = np.array(data)
        # count_choice = np.random.choice(np.arange(data.shape[0]), size=int(data.shape[0] * 0.01), replace=False)
        # count_choice = range(int(data.shape[0] * 0.001))
        count_choice = range(600)
        # bandwidth = estimate_bandwidth(X=data[count_choice, :], quantile=0.15)
        # print("bandwidth:                                    {}".format(bandwidth))
        meanshift = Birch(threshold=0.005, branching_factor=50, n_clusters=10).fit(data[count_choice, :])
        K = meanshift.predict(data)
        global_K_test = K
...
def meanshift_cluster_train(data_in):
    global var_train
    global global_K_train
    # global ratio
    n_samples = data_in.shape[0]
    var_train += 1
    if var_train == 1:
...
def one_stage_saak_trans_test(datasets=None, depth=0, components_pca=0):
    ...
    data_cluster, K = meanshift_cluster_test(datasets)
...
def one_stage_saak_trans_train(datasets=None, depth=0, components_pca=0):
    ...
    input_channels = datasets.shape[1]
    index = [datasets.shape[1], datasets.shape[2], datasets.shape[3]]
    data_cluster, K = meanshift_cluster_train(datasets)
    n_clusters = len(np.unique(K))
    print("one_stage_saak_trans: number of clusters: {}".format(n_clusters))
    data_n = []
    for i in range(n_clusters):
