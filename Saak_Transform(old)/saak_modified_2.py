...
def multi_stage_saak_trans(components_pca):
    ...
    for i in range(num):
        ...
        data, output = one_stage_saak_trans(data, depth=i, components_pca=components_pca[i])
        # train_filters.append(train_filt)
        outputs.append(output)
        # test_data, test_filt, test_output = one_stage_saak_trans(test_data, depth=i, components_pca=components_pca[i])
        # test_filters.append(test_filt)
        # test_outputs.append(test_output)
        print('')

    return train_labels, test_labels, outputs
...
def main():
    ...
    start_time = time()
    train_labels, test_labels, outputs = multi_stage_saak_trans(components_pca=components_PCA)
    mid_time = time()
    ...
    for output in outputs:
        output = output[:train_size].data.numpy()
