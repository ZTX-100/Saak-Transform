...
def fit_pca_shape(datasets, depth):
    ...
    # shape reshape
    data = np.reshape(data_lattice, (data_lattice.shape[0] * data_lattice.shape[1], data_lattice.shape[2], 3, 3))
...
def ret_filt_patches(aug_anchors, input_channels):
    shape = aug_anchors.shape[1] / 9
    num = aug_anchors.shape[0]
    shape = int(shape)
    filt = np.reshape(aug_anchors, (num, shape, 9))

    # reshape to kernels, (# output_channels,# input_channels,2,2)
    filters = np.reshape(filt, (num, shape, 3, 3))
...
def one_stage_saak_trans(datasets=None, depth=0, components_pca=0):
    ...
    relu_output, filt = conv_and_relu(filters, datasets, stride=3)
...
def main():
    ...
    for filt in train_filters:
        ...
        relu_output, filt = conv_and_relu(filter_numpy, test_data, stride=3)
