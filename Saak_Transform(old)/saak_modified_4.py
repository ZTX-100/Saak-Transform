...
def saak_coefficients_test(datasets=None, components_pca=0, input_channels=0, depth=0):
...
def one_stage_saak_trans_test_1(datasets=None, depth=0, components_pca=0):
    ...
    data = saak_coefficients_test(datasets[:int(datasets.shape[0] / block_size), :, :, :], components_pca, input_channels, depth)
    # dataset = np.empty([datasets.shape[0], data.shape[1], data.shape[2], data.shape[3]])
    # dataset[0, :, :, :] = data
    for i in range(1, block_size):
        data = np.vstack((data, saak_coefficients_test(datasets[int(datasets.shape[0] / block_size) * i:int(datasets.shape[0] / block_size) * (i + 1), :, :, :], components_pca, input_channels, depth)))
    relu_output = list(data)
    return data, relu_output


def one_stage_saak_trans_test_2(datasets=None, depth=0, components_pca=0):
...
def one_stage_saak_trans_train(datasets=None, labels=None, depth=0, components_pca=0):
    ...
    # dataset[0, :, :, :] = data
    for i in range(10):
        data_n.append(saak_coefficients_train(dataset[i], components_pca, input_channels, depth))
...
def multi_stage_saak_trans_1(components_pca):
    train_outputs = []
    test_outputs = []
    ...
    for i in range(num):
        ...
        train_data, train_output = one_stage_saak_trans_test_1(train_data, depth=i, components_pca=components_pca[i])
        train_outputs.append(train_output)
        test_data, test_output = one_stage_saak_trans_test_2(test_data, depth=i, components_pca=components_pca[i])
...
def multi_stage_saak_trans_2(components_pca, train_cluster, test_cluster):
    train_outputs = []
    test_outputs = []
    ...
    for i in range(num):
        print("{} stage of saak transform: ".format(i))
        train_data, train_output = one_stage_saak_trans_train(train_data, train_cluster, depth=i, components_pca=components_pca[i])
        train_outputs.append(train_output)
        test_data, test_output = one_stage_saak_trans_train(test_data, test_cluster, depth=i, components_pca=components_pca[i])
        test_outputs.append(test_output)
        print('')
...
def main_1():
    components_PCA = [3, 4, 7, 3, 0]
    train_size = 10000
    test_size = 10000

    # start_time = time()
    train_data, train_labels, test_data, test_labels, train_outputs, test_outputs = multi_stage_saak_trans_1(components_pca=components_PCA)
    # mid_time = time()
    ...
    train_cluster = classifier.predict(train_coefficients_pca)
    test_cluster = classifier.predict(test_coefficients_pca)
...
def main_2(train_cluster, test_cluster):
    components_PCA = [3, 4, 7, 6, 8]
    train_size = 10000
    test_size = 10000

    start_time = time()
    train_data, train_labels, test_data, test_labels, train_outputs, test_outputs = multi_stage_saak_trans_2(components_pca=components_PCA, train_cluster=train_cluster, test_cluster=test_cluster)
    mid_time = time()
...
if __name__ == '__main__':
    train_cluster, test_cluster = main_1()
    main_2(train_cluster, test_cluster)
