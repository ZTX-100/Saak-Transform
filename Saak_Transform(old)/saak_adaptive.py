...
def conv_and_relu(filters, datasets, stride=2):
    ...
    # Convolution
    output = F.conv1d(data, filt, stride=stride)
...
def kmeans_cluster(data_in):
    ...
    K = kmeans.labels_
    data_0 = [dat for dat, k in zip(data, K) if k == 0]
    data_1 = [dat for dat, k in zip(data, K) if k == 1]
...
def one_stage_saak_trans(datasets=None, depth=0, components_pca=0):
    ...
    if var == 0 or var == 1:
        ...
        data_flatten = datasets
    var += 1

    data_flatten_0, data_flatten_1 = kmeans_cluster(data_flatten)
    data_0 = np.expand_dims(data_flatten_0, axis=1)
    data_1 = np.expand_dims(data_flatten_1, axis=1)
    print("one_stage_saak_trans: data_flatten_0.shape: {}".format(data_flatten_0.shape))
    print("one_stage_saak_trans: data_flatten_1.shape: {}".format(data_flatten_1.shape))
    print("one_stage_saak_trans: data_0.shape: {}".format(data_0.shape))
    print("one_stage_saak_trans: data_1.shape: {}".format(data_1.shape))
    ...
    # comps_0 = comps_0[:components_pca + 1, :]
    comps_complete_0 = comps_complete_0[:(components_pca + 1), :]
    # comps_1 = comps_1[:components_pca + 1, :]
    comps_complete_1 = comps_complete_1[:(components_pca + 1), :]
    filters_augment_0 = np.expand_dims(comps_complete_0, axis=1)
    filters_augment_1 = np.expand_dims(comps_complete_1, axis=1)

    # print("one_stage_saak_trans: comps_0: {}".format(comps_0.shape))
    print("one_stage_saak_trans: comps_complete_0: {}".format(comps_complete_0.shape))
    # print("one_stage_saak_trans: comps_1: {}".format(comps_1.shape))
    print("one_stage_saak_trans: comps_complete_1: {}".format(comps_complete_1.shape))
    # get filter, (7,1,2,2)
    # filters_0 = ret_filt_patches(comps_0, input_channels)
    # filters_augment_0 = ret_filt_patches(comps_complete_0, input_channels)
    # filters_1 = ret_filt_patches(comps_1, input_channels)
    # filters_augment_1 = ret_filt_patches(comps_complete_1, input_channels)
    # print("one_stage_saak_trans: filters: {}".format(filters_0.shape))
    print("one_stage_saak_trans: filters_augment: {}".format(filters_augment_0.shape))

    # output (60000,7,14,14)
    # output_0, relu_output_0 = conv_and_relu(filters_0, data_0, stride=2)
    output_augment_0, relu_output_augment_0 = conv_and_relu(filters_augment_0, data_0, stride=2)
    # output_1, relu_output_1 = conv_and_relu(filters_1, data_1, stride=2)
    output_augment_1, relu_output_augment_1 = conv_and_relu(filters_augment_1, data_1, stride=2)
    # output = np.append(np.squeeze(output_0.data.numpy()), np.squeeze(output_1.data.numpy()), axis=0)
    # print("output.shape: {}".format(output.shape))
    # output = output.reshape(datasets.shape[0], -1, output.shape[1])
    # output = np.transpose(output, axes=[1, 0])
    # output = output.reshape((datasets.shape[0], components_pca + 1, 32 / np.power(2, depth + 1), 32 / np.power(2, depth + 1)))
    # print("one_stage_saak_trans: output.shape: {}".format(output.shape))
    # output = list(output)
    relu_output_augment = np.append(np.squeeze(relu_output_augment_0.data.numpy()), np.squeeze(relu_output_augment_1.data.numpy()), axis=0)
    print(relu_output_augment.shape)
    relu_output_augment = relu_output_augment.reshape([-1, 2 * 2 * relu_output_augment.shape[1]])
    print(relu_output_augment.shape)
    data = relu_output_augment
    output = list(relu_output_augment)
    print("one_stage_saak_trans: output: {}".format(data.shape))

    # output S/P
    # output_position = np.empty([data.shape[0], 2 * data.shape[1] - 1, data.shape[2], data.shape[3]])
    # for i in range(data.shape[0]):
    #     for j in range(data.shape[1]):
    #         for m in range(data.shape[2]):
    #             for n in range(data.shape[3]):
    #                 if j == 0:
    #                     output_position[i, j, m, n] = data[i, j, m, n]
    #                 else:
    #                     if data[i, j, m, n] >= 0:
    #                         output_position[i, 2 * j - 1, m, n] = data[i, j, m, n]
    #                         output_position[i, 2 * j, m, n] = 0
    #                     elif data[i, j, m, n] < 0:
    #                         output_position[i, 2 * j - 1, m, n] = 0
    #                         output_position[i, 2 * j, m, n] = -data[i, j, m, n]
